...
public class MovingPlatform : MonoBehaviour
{
    ...
    IEnumerator MovePlatform()
    {
        while (true)
        {
            while (Vector3.Distance(platform.transform.position, tragetPosition) > 0.1f)
            {
                platform.transform.position = Vector3.MoveTowards(platform.transform.position, tragetPosition, speed * Time.deltaTime);
                yield return null;
            }
